...
translitFromBlock :: Text -> Maybe Text
translitFromBlock syllable =
...
translitFromBlocks :: Text -> Text
translitFromBlocks =
    replace "   " "  " "g"
  . join " "
  . fromMaybes
  . L.map (\v -> if v == " " then Just v else translitFromBlock v)
  . split ""
...
blockFromTranslit :: Text -> Maybe Char
blockFromTranslit syllable =
  case match "^([bcdghjklmnprst]*)([aeiouyw]*)([bcdghjklmnprst]*)$" "i" syllable of
    Null -> Nothing
    Nullable [_,"","",""] -> Nothing
    Nullable [_,a ,b ,c ] ->
      Just . fromCharCode $
        blockCode (aux a initial)
                  (aux b vowel  )
                  (aux c final  )
  where aux x = fromMaybe 0 . lookT (if x == "" then "_" else x)

blocksFromTranslit :: Text -> Text
blocksFromTranslit =
    join ""
  . mapMaybe (\c -> if c == "" then Just " " else (`cons` "") <$> blockFromTranslit c)
  . split " "
...
replace = ffi "%4.replace(%1,%2,%3)"

split :: Text -> Text -> [Text]
split = ffi "%2.split(%1)"

match :: Text -> Text -> Text -> Nullable [Text]
match = ffi "%3.match(%1,%2)"
